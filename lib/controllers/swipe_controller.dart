...
class SwipeController extends GetxController {
  ...
  Future<void> loadSwipedUsersFromDB() async {
    try {
      ...
      acceptedUsers.value = acceptedMaps.map((m) => DummyUser.fromMap(m)).toList();

      final rejectedMaps = await dbHelper.getSwipesByStatus('rejected');
      rejectedUsers.value = rejectedMaps.map((m) => DummyUser.fromMap(m)).toList();
    } catch (e) {
  ...
  Future<void> fetchUsersFromAPI() async {
    try {
      ...
      final response = await http.get(Uri.parse('https://dummyjson.com/users?limit=100'));
